      ...
      <div className="p-5">
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {visible.map((point, i) => {
            ...
            return (
              <div
                key={i}
                className={`sensor-card p-4 rounded-xl border border-outline-variant/30 hover:border-outline-variant/50 transition-all ${gridVisible ? 'is-visible' : ''}`}
                style={{
                  background: '#0e0e0f',
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-2.5">
                  {[
                    ...
                    <div key={m.label} className="flex items-center gap-1.5">
                      <span className="material-symbols-outlined text-outline/50 text-[12px]">{m.icon}</span>
                      <div>
                        <p className="text-[8px] uppercase tracking-wider text-outline/50 leading-none mb-0.5">{m.label}</p>
                        <p className="text-[11px] font-semibold text-on-surface leading-none">{m.value}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {(point.accident || point.event) && (
                  <div className="flex gap-1.5 mt-3 flex-wrap">
                    {point.accident && (
                      <span className="text-[9px] px-2 py-0.5 rounded-full bg-error/10 border border-error/30 text-error font-semibold">
                        🚨 Accident
                      </span>
                    )}
                    {point.event && (
                      <span className="text-[9px] px-2 py-0.5 rounded-full bg-tertiary/10 border border-tertiary/30 text-tertiary font-semibold">
                        🎪 Event
                      </span>
                    )}
                  </div>
        ...
        {hiddenCount > 0 && (
          <button
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-outline-variant/20 text-xs font-medium text-on-surface-variant hover:text-primary hover:border-primary/30 transition-all"
          >
            <span className={`material-symbols-outlined text-[16px] transition-transform duration-300 ${expanded ? 'rotate-180' : ''}`}>
              expand_more
            </span>
            {expanded ? 'Show less' : `Show ${hiddenCount} more sensor${hiddenCount > 1 ? 's' : ''}`}
          </button>
        )}
      </div>
    </div>
